...
// Endpoint para consultar equipos del empleado - CORREGIDO
if (isset($_POST['peticion']) && $_POST['peticion'] === 'consultar_equipos_empleado' && isset($_POST['cedula_empleado'])) {
    ...
    // Usar el modelo Equipo en lugar de Bien
    $data = [
    ...
    if ($json['resultado'] === 'equipos_por_empleado') {
        echo json_encode(['resultado' => 'consultar_equipos_empleado', 'datos' => $json['datos']]);
    } else {
        // En caso de error, devolver array vacío
        echo json_encode(['resultado' => 'consultar_equipos_empleado', 'datos' => []]);
    }
    exit;
}
...
if (is_file("view/" . $page . ".php")) {
    ...
    if (isset($_POST["solicitud"]) && $_POST["motivo"] != NULL) {
        ...
        try {
            // Validar motivo - CORREGIDO: aumentar límite a 200 caracteres
            $motivo = trim($_POST["motivo"]);
            if (empty($motivo) || strlen($motivo) < 5 || strlen($motivo) > 200) {
                throw new Exception("El motivo debe tener entre 5 y 200 caracteres");
            }
            
            // Validar caracteres permitidos
            if (preg_match("/^[0-9a-zA-ZáéíóúüñÑçÇÁÉÍÓÚÜ ,.\-()]{5,200}$/", $motivo) == 0) {
                throw new Exception("El motivo contiene caracteres no permitidos");
            }

            // Generar ID único para la solicitud
            $nro_solicitud = generarID("PR", substr($motivo, 0, 2));
            $solicitud->set_nro_solicitud($nro_solicitud);
            $solicitud->set_cedula_solicitante($datos["cedula"]);
            $solicitud->set_motivo($motivo);
            ...
            if ($resultado['bool'] == 1) {
                ...
                $msgN = "Nueva solicitud #{$nro_solicitud} creada por " .
                    ...
                    "(" . $_SESSION['user']['nombre_usuario'] . "): " . $motivo;
