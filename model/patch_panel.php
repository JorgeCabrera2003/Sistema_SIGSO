...
class patch_panel extends Conexion {
    ...
    private function reportePatchPanelPorNro($id_piso) {
        $this->conex = new Conexion("sistema");
        $this->conex = $this->conex->Conex();

        $dato = [];
        $stm = null; // Inicializar para evitar error de referencia

        try {
            // Llamar el procedimiento con el id_piso directamente
