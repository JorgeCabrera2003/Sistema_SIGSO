...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 06-07-2025 a las 16:26:19
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_reporte_patch_panel` (IN `p_id_piso` INT)   BEGIN
    SELECT 
        ...
        (SELECT COUNT(*) 
         ...
         WHERE b.estatus = 1 AND p.id_piso = p_id_piso) AS 'Cantidad Patch Panel'
    FROM
        (SELECT SUM(p.cantidad_puertos) AS total_puertos
         FROM patch_panel p
         JOIN bien b ON b.codigo_bien = p.codigo_bien
         WHERE b.estatus = 1 AND id_piso = p_id_piso) AS total,
        
        (SELECT COUNT(*) AS puertos_ocupados
         FROM patch_panel p
         JOIN bien b ON b.codigo_bien = p.codigo_bien
         JOIN punto_conexion pc ON pc.codigo_patch_panel = p.codigo_bien
         WHERE b.estatus = 1 AND id_piso = p_id_piso) AS ocupado;
END$$
